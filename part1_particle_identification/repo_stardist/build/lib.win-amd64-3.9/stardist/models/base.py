...
class StarDistBase(BaseModel):
    ...
    def _predict_setup(self, img, axes, normalizer, n_tiles, show_tile_progress, predict_kwargs):
        """ Shared setup repo_stardist between `predict` and `predict_sparse` """
        if n_tiles is None:
            n_tiles = [1]*img.ndim
        try:
    ...
    def predict_instances_prob(self, img, axes=None, normalizer=None,
                          sparse=True,
                          prob_thresh=None, nms_thresh=None,
                          scale=None,
                          n_tiles=None, show_tile_progress=True,
        ...
        if return_predict:
            return res_instances,prob,tuple(res[:-1])
        else:
            return res_instances,prob
